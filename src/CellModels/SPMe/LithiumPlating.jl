# =====================================================================================================================
# LithiumPlating.jl
# Lithium plating degradation models for SPMe
# Contains:
# 1. NoPlating: Plating disabled
# 2. IrreversiblePlating: All plated lithium is permanently lost (dead)
# 3. PartiallyReversiblePlating: Plated lithium can be stripped, but fraction becomes dead via SEI-coupled damage
# =====================================================================================================================
...
function NoPlating(; name, p, s, g)
    ...
    @named L_sei = RealInputArray(nin=N) # Interface consistency
    ...
    @variables begin
        ...
        (c_plating(t))[1:N] = 0
        (c_dead(t))[1:N] = 0
        (j_stripping(t))[1:N], [guess=j_strip0 * ones(N)]
        (ϕf(t))[1:N], [guess=zeros(N)]

        c_plating_x(t), [guess=0.0]
        c_dead_x(t), [guess=0.0]
        j_stripping_x(t), [guess=j_strip0]
        ϕf_x(t), [guess=0.0]
    end
...
"""
function IrreversiblePlating(; name, p, s, g)
    ...
    @variables begin
        ...
        (j_stripping(t))[1:N], [guess=j_strip0 * ones(N)]
        (ϕf(t))[1:N], [guess=zeros(N)]
        (η_plating(t))[1:N], [guess=zeros(N)]
        (η_stripping(t))[1:N], [guess=zeros(N)]
        (j0_plating(t))[1:N], [guess=zeros(N)]
        (j0_stripping(t))[1:N], [guess=zeros(N)]

        c_plating_x(t), [guess=0.0]
        c_dead_x(t), [guess=0.0]
        L_plating_x(t), [guess=0.0]
        L_dead_x(t), [guess=0.0]
        j_stripping_x(t), [guess=j_strip0]
        ϕf_x(t), [guess=0.0]
        η_plating_x(t), [guess=0.0]
        η_stripping_x(t), [guess=0.0]
    end
...
Uses SEI thickness to calculate the rate at which active plating becomes dead.
...
"""
function PartiallyReversiblePlating(; name, p, s, g)
    ...
    # Structural damage coupling based on SEI thickness
