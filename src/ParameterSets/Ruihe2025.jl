...
n = SolidParticleParameters(
    ...
    c₀ = 28543, # Initial electrode concentration in mol*m^-3
    c₊ = 32544, # Maximum electrode concentration in mol*m^-3
...
p = SolidParticleParameters(
    ...
    c₀ = 12727, # Initial electrode concentration in mol*m^-3
    c₊ = 52787, # Maximum electrode concentration in mol*m^-3
    # Open-circuit potential in V
...
e = ElectrolyteParameters(
    ...
    Dₑ= (c,T) -> (8.794e−11 * (c/1000)^2 − 3.972e−10 * (c/1000) + 4.862e−10)*exp(17000/8.314*(1/298.15-1/T)), # Diffusivity in m^2*s^-1
    σₑ= (c,T) -> (0.1297 * (c/1000)^3 − 2.51*(c/1000)^1.5 + 3.329*(c/1000))*exp(17000/8.314*(1/298.15 - 1/T)), # Conductivity in S*m^-1
