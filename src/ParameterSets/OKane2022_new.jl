...
function plating_exchange_current_density_OKane2020(c_e, c_Li, T)
    """
    ...
    return F * k_plating * c_e

end

function stripping_exchange_current_density_OKane2020(c_e, c_Li, T)
    """
    ...
    return F * k_plating * c_Li

end

function SEI_limited_dead_lithium_OKane2022(L_sei)
    """
...
function graphite_LGM50_diffusivity_Chen2020(sto, T)
    """
    ...
    arrhenius = exp(E_D_s / R * (1 / 298.15 - 1 / T))

    return D_ref * arrhenius

end

function graphite_LGM50_electrolyte_exchange_current_density_Chen2020(c_e, c_s_surf, c_s_max, T)
    """
    ...
    E_r = 35000
    arrhenius = exp(E_r / R * (1 / 298.15 - 1 / T))

    return m_ref * arrhenius * c_e^0.5 * c_s_surf^0.5 * (c_s_max - c_s_surf) ^ 0.5

end

function graphite_volume_change_Ai2020(sto)
    """
...
function graphite_cracking_rate_Ai2020(T_dim)
    """
    ...
    arrhenius = exp(Eac_cr / R * (1 / T_dim - 1 / 298.15))
    return k_cr * arrhenius

end

function nmc_LGM50_diffusivity_Chen2020(sto, T)
    """
...
function nmc_LGM50_ocp_Chen2020(sto)
    """
...
function nmc_LGM50_electrolyte_exchange_current_density_Chen2020(c_e, c_s_surf, c_s_max, T)
    """
...
function volume_change_Ai2020(sto)
    """
...
function cracking_rate_Ai2020(T_dim)
    """
...
function electrolyte_diffusivity_Nyman2008_arrhenius(c_e, T)
    """
    ...
    E_D_c_e = 17000
    arrhenius = exp(E_D_c_e / R * (1 / 298.15 - 1 / T))

    return D_c_e * arrhenius
end

function electrolyte_conductivity_Nyman2008_arrhenius(c_e, T)
    """
    ...
    E_sigma_e = 17000
    arrhenius = exp(E_sigma_e / R * (1 / 298.15 - 1 / T))
