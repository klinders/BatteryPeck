...
function build_pack_geometry(; 
    ...
    # Apply routing and encasement rule engine
    ...
    for i in firstindex(tms_passes):(lastindex(tms_passes)-1)
        ...
        # Sort U-turn points purely vertically
        sort!(turn_pts, by = p -> p[2])
        u_turns[i] = turn_pts
        direction *= -1
    end
    
    # Generate and exact-clip straightaways
    ...
    for i in eachindex(tms_passes)
        ...
        # Apply exact point-to-point bounding logic
        if direction == 1 
            if i > 1
                # Connect to top of left U-turn
                pass_min_x = u_turns[i-1][end][1] 
            end
            if i < lastindex(tms_passes)
                # Connect to bottom of right U-turn
                pass_max_x = u_turns[i][1][1]     
            end
        else 
            if i > 1
                # Connect to top of right U-turn
                pass_max_x = u_turns[i-1][end][1] 
            end
            if i < lastindex(tms_passes)
                # Connect to bottom of left U-turn
    ...
    # Flatten array and remove topological duplicates seamlessly
    ...
    # Assign dynamic shadow width mapping for hexagonal versus square
    ...
    for c_idx in eachindex(cell_coords)
        ...
        # Shadows from connected TMS nodes
        ...
        # Normalise and merge overlapping shadow arcs
