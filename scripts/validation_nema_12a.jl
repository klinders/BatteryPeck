...
function run_figure_12a_validation(t_hg_end)
    ...
    for (i, h_mm) in enumerate(heights_mm)
        ...
        println("  -> h = $h_mm mm  |  RMSE: $(round(rmse, digits=3)) °C  |  R²: $(round(r2, digits=4))")

        # Overlay simulation data on main plot with "Sim."
        plot!(p_main, sol.t, T_max_sim_C, label="Sim. $h_mm mm", color=matlab_colors[i], linewidth=2)
        # Overlay CFD validation data on main plot with "CFD"
        plot!(p_main, t_val, y_val_C, label="CFD $h_mm mm", color=matlab_colors[i], linestyle=:dash, linewidth=2)

        # Create dedicated subplot for current height with "Sim." and Celsius labels/limits
        p_sub = plot(
            sol.t, T_max_sim_C, 
            label="Sim.", color=matlab_colors[i], linewidth=2, 
            title="Height = $h_mm mm", xlabel="Time (s)", ylabel="Temperature (°C)", 
            grid=true, ylims=(20, 65), legend=:topleft
        )
        
        # Overlay validation data on subplot (keeping "CFD data")
        plot!(p_sub, t_val, y_val_C, label="CFD data", color=matlab_colors[i], linestyle=:dash, linewidth=2)
        
        # Annotate subplot with calculated error metrics (y-pos 30 maps equivalently to 303K)
    ...
    # Compile individual plots into final grid layout leaving bottom right spot empty, with increased margin
...
if export_vector_image
    output_filename = "validation_nema_12a_plot.svg" 
