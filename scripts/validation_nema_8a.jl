...
function run_and_validate(rate_name, q_func, t_hg_end, val_filename)
    ...
    # Initialise plot with simulation data and explicit margins to prevent clipping
    plt = plot(
        t_sim, T_max_sim_C, 
        label="LPTN simulation", 
        linewidth=2, color=:blue,
        xlabel="Time (s)", ylabel="Temperature (°C)",
        title="$rate_name ambient cooling",
...
if export_vector_image
    output_filename = "validation_nema_8a_plot.svg" 
